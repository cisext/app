exUtilsVersion = '1.0.3 beta';

/* 
ver 1.02 du 11 septembre 2022
- update : added a callback argument on the fireEval function to handle cases where the function to execute is asynchronous.

var 1.0.3 du 3 ocotbre 2022
- add : removeAccent function
*/
...
/* async function evalNinox (fn, recordId) {

  try {
    console.log("NINEXT fireEval(" + fn + ",\n" + recordId + ")");
    ...
    if (compile.hasErrors())
      return { error: "Erreur d'expression : " + compile.errorMessage() };
    var result = null;
          ...
          return e
            ...
            : i
              ? compile.evaluate(database, i, function (error, t) {
                ...
                return result = t, result, callback && callback(error, t);
              })
...
window.exUtilsNx = (function () {
  console.log("NINEXT exUtilsNx.constructor");
  return {
    ...
    fireEval: function (fn, recordId, callback) {
      try {
        var res = { result: null, error: null };
        console.log("NINEXT fireEval(" + fn + ",\n" + recordId + ")");

        var type = database.typeOf(this.getId(recordId));
        var compile = queries.parseSystem(database.schema, type, unescape("(" + fn + ")"),
          {}
        );
        if (compile.hasErrors())
          compile = queries.parseHuman(database.schema, type, unescape(fn), {});
        if (compile.hasErrors()) {
          res.error = "Erreur d'expression : " + compile.errorMessage();
          callback && callback(res.error, res.result);
          return res;
        }
        database.loadNode(recordId, function (e, i) {
          if (e) {
            res.error = "Failed to load record: " + e;
            callback && callback(res.error, res.result);
            return res;
          }
          else return compile.evaluate(database, i, function (error, t) {
            res.error = e ? "Failed to evaluate expression: " + error : null;
            res.result = t;
            callback && callback(res.error, res.result);
            return res;
          })
        ...
        res.error = err.message + " à la ligne " + err.line + ", colonne " + err.column;
        callback && callback(res.error, res.result);
        return res;
      }
    },
    ...
    extractNxFonctionInScript: function (fnName, script, field) {
      ...
      if (compile.hasErrors()) {
        return console.log("NINEXT extractNxFonctionInScript : Erreur d'expression : " + compile.errorMessage(),
          fnName, script, field), null;
      }
      var r = null;
    ...
    findNxFunctionInField: function (fnName, element) {

      //retrieve component by Id or by child element 
      var cpn = typeof element == 'sring' ? this.findNxComponentFromElementId(element) : $(element).closest(".component");
    ...
    fireNxFunction: function (fnName, element) {
    ...
    getBoundingRect: function (element, relativeTo = null) {
      var relativeRect = relativeTo ? relativeTo.getBoundingClientRect() : { left: 0, top: 0, right: 0, bottom: 0 };
      var elemRect = element.getBoundingClientRect();
      var r = {
        top: elemRect.top - relativeRect.top,
        right: relativeRect.right - elemRect.right,
        bottom: relativeRect.bottom - elemRect.bottom,
        left: elemRect.left - relativeRect.left,
